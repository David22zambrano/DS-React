...
export interface AdjuntarProps<T> {
  fecthDB: (archivos: T[]) => void;
  compact?: boolean;
  sx?: SxProps;
  transformFile: (file: File) => T; 
}

export const AdjuntarArchivoGenerico = <T,>({
  compact,
  sx,
  fecthDB,
  transformFile,
  ...
  const [archivos, setArchivos] = useState<
    { file: File; transformed: T; progress: number; loadingComplete: boolean }[]
  >([]);
    ...
    if (files) {
      setArchivos((prev) => [
        ...
        ...Array.from(files).map((file) => ({
          file,
          transformed: transformFile(file),
          progress: 0,
  ...
  const Eliminar = useCallback(
    (index: number) => {
      setArchivos((prevFiles) => prevFiles.filter((_, idx) => idx !== index));
    },
    [setArchivos]
  );
  ...
  const controlEventoAdjuntar = (e: React.MouseEvent) => {
    e.stopPropagation();
    fecthDB(archivos.map((e) => e.transformed));
    setArchivos([]);
  };
